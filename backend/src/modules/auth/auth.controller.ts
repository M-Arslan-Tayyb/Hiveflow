...
const magicLinkLogin = asyncHandler(async (req: Request, res: Response) => {
  const { user, accessToken, refreshToken, project } =
    await authService.magicLinkLogin(req.query.token as string | undefined);
  ...
  res.status(200).json(
    new ApiResponse(200, "Magic link login successful", {
      user,
      accessToken,
      refreshToken,
      project,
