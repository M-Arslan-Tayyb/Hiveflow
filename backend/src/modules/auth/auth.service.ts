...
const logoutUser = async (refreshToken: string | undefined) => {
  if (!refreshToken) {
    throw new ApiError(401, "Refresh token not found");
  }
...
const verifyEmail = async ({ token }: VerifyEmailInput) => {
  // 1. Hash token
  const hashedToken = crypto
    .createHash("sha256")
    .update(token)
...
const magicLinkLogin = async (token: string | undefined) => {
  ...
  if (!invite) throw new ApiError(400, "No valid project invite found");
