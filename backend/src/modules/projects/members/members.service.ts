...
interface InviteProjectMemberInput {
  projectId: string;
  orgId: string;
  email: string;
  fullName?: string;
...
const addMember = async ({ projectId, orgId, userId, role }: AddMemberInput) => {
  // User exists?
...
const removeMember = async ({
  ...
  if (targetUserId === requestingUserId && targetMember.role === "MANAGER") {
    ...
    if (managerCount === 1) {
      throw new ApiError(
        403,
        "Cannot remove yourself as the only project manager"
      );
    }
  }
...
const inviteProjectMember = async ({
  ...
  if (existingInvite)
    throw new ApiError(
      409,
      "User already has a pending invite for this project. The invite expires in 7 days."
    );
